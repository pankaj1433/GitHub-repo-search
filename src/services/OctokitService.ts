...
export class OctokitService {
  ...
  async searchRepositories(
    ...
    try {
      const response = await this.octokitClient.request<any>(
  ...
  async getRepoReadme(owner: string, repo: string): Promise<string> {
    try {
      const response = await this.octokitClient.rest.repos.getReadme({
        owner,
        repo,
        mediaType: {
          format: "html",
      ...
      return response.data as unknown as string; // TypeScript issue with data type
    } catch (error) {
